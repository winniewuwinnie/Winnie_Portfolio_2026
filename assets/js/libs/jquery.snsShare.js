...
jQuery.fn.snsShare = function (message, url) {
   ...
   * @type {string[]}
   */
  types = ['facebook', 'google+', 'line', 'twitter', 'plurk', 'linkedin'];
    ...
    jQuery(this).click(function () {
      ...
      if (typeof (snsType) === 'string' && jQuery.inArray(snsType, types) !== -1) {
        ...
        switch (snsType) {
          ...
          case 'twitter':
            element = getAtagElement();
            element.href = protocol + '//twitter.com/intent/tweet?url=' + encodeURIComponent(message + ' ' + url);
            element.dispatchEvent(makeMouseClickEvent());
            break;
          case 'plurk':
            element = getAtagElement();
          ...
          case 'linkedin':
            element = getAtagElement();
            element.href = protocol + '//www.linkedin.com/shareArticle?mini=true&url=' + url + '&title=' + message;
            element.dispatchEvent(makeMouseClickEvent());
